...
function Shows() {
  ...
  function handleAddFav(e, id) {
    e.preventDefault();
    let _addFav = { ...addFav };
    _addFav.ids = _addFav.ids || {};
    _addFav.ids[id] = (_addFav.ids[id] || 0) + 1;
    _addFav.totalItems = (_addFav.totalItems || 0) + 1;
    setAddFav(_addFav);
    navigate("/");
  }
  ...
  return (
    <>
      <Navbar
        favAdd={addFav && addFav.totalItems}
        allData={allData}
      />
      {currentPosts.length === 0 ? (
        <div className="gifSetting">
          <img
            src="https://icons8.com/preloaders/preloaders/1493/Settings.gif"
            alt="Animation"
          />
        </div>
      ) : (
          ...
          <div className="bg_color1">
            ...
            <div className="d-flex justify-content-center">
              <div className="sortingItems">
                <div>
                  ...
                  >
                    <i className="fas fa-border-all fs-4"></i>
                  </button>
                  <button
            ...
            <div className="row row-cols-1 row-cols-md-4 g-4 m-3 mt-0">
              {currentPosts.map((data, i) => (
                ...
                >
                  <div
                    className="card h-100 HoverEff"
                    onClick={() => handleDetails(data.id)}
                  >
                    <img
                      src={data.image.original}
                      className="card-img-top img-hight position-relative"
                      loading="lazy"
                      alt="..."
                    />
                    <button
                      onClick={(e) => {
                        handleAddFav(e, data.id);
                      }}
                      className="addFavBtn"
                    >
                      <h1>+</h1>
                    ...
                    <div className="card-body">
                      <p className="card-text m-0 text-secondary mt-2">
                        <i className="fas fa-star text-warning"></i>{" "}
                        {data.rating.average}
                      </p>
                      <div className="mt-4">
                        <h4 className="card-title fs-4">
                          <small className="fs-6">{i + 1}. </small> {data.name}
                        </h4>
                        <div className="d-flex flex-wrap">
                          <p className="card-text m-0 text-secondary">
                            {data.premiered}
                          </p>
                          <p className="card-text m-0 text-secondary mx-4">
                            {data.runtime}min/episode
                          </p>
                        </div>
                      </div>
                    </div>
