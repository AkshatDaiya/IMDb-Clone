            ...
            <div className="watchlistGB pt-3 mt-4">
                <div className="watchlist">
                    <div className="label">
                        ...
                        <h6 className='fs-5 fw-bolder px-2' style={{ color: '#b1a5a5' }}><i className="fa-solid fa-lock"></i> PRIVATE</h6>
                    </div>
                    ...
                    {watchListData.length === 0 ? (
                        ...
                        watchListData.map((data) => (
                            <div key={data.id} className="card m-2">
                                <div className="row g-0 rounded-0">
                                    <div className="col-md-4">
                                        <img src={data.image.original} className="watchListIMG img-fluid rounded-start" alt={data.name} />
                                    </div>
                                    <div className="col-md-8 position-relative">
                                        <div className="card-body watchListData">
                                            <Link to="/showDetails">
                                                <h5 className="card-title m-0" onClick={() => handleDetails(data.id)}>{data.name}</h5>
                                            </Link>
                                            <p className="m-0">
                                                <small>{data.premiered} | {data.runtime}min/ep | {data.genres.join(', ')}</small>
                                            </p>
                                        ...
                                        <button className="StyleBTN3 position-absolute" onClick={() => handleDetails(data.id)}>
                                            More Details
                                        </button>

                                        <button className="StyleBTN4 position-absolute" onClick={() => handleDelete(data._id)}>
                                            Remove
                                        </button>
                                    </div>
                                </div>
