...
function Carousel({ genresData }) {
    ...
    return (
        <div className="container py-5">
            ...
            <Slider {...settings}>
                {genresData ? genresData.map((data, index) => (
                    <div key={`${data.id}-${index}`}>
                        <div className="card" style={{ width: "17rem", height: "34rem" }}>
                            <img src={data.image.original} className="card-img-top" alt="..." />
                            <div className="card-body position-relative">
                                <h5 className="m-0">{data.name}</h5>
                                <Link to={`/showDetails`}>
                                    <button className='StyleBTN2 position-absolute' onClick={() => { handleDetails(data.id) }}>More Details</button>
                                </Link>
                            </div>
                        </div>
                    </div>
